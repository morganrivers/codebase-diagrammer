"""
drawio_common.py — shared primitives for draw.io diagram generation.

build_diagram.py lays a node/edge graph out with Graphviz `dot`, then bakes the
resulting node positions and routed edge waypoints into an editable .drawio.
Everything below the level of one diagram's own choices (the pixel emission of
a vertex, of an edge with waypoints, the tooltip encoding and the mxfile
wrapper) lives here, so a generator that places its own rectangles rather than
calling `dot` still emits identical XML.

Node dicts use the schema: id, lbl, type, grp, file, desc, robot, link, w, h.
Edge dicts use: from, to, label, kind, tip, link.
"""
